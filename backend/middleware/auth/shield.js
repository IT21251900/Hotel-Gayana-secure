...
class Shield {
    ...
    authorize(req, res, next, permissions) {
        if (permissions.some(e => req.user.permissions.indexOf(e) > -1)) {
            ...
            return res.status(403).send({ code: 1004, message: 'Authorization Failed' });
        }
    }
            ...
            if (error) {
                ...
                switch (userType) {
                    ...
                    case SETTINGS.USERS.CUSTOMER:
                        ...
                        if (!customer) {
                            log.error('Authentication Failed - Customer Deleted or Deactivated');
                            return res.status(403).send({
                                code: 403,
                                message: 'Authentication Failed - Customer Deleted or Deactivated',
                            });
                        }
                        break;
                }
    ...
    extractBearerToken(req, res, next, publicKey, permissions, authHeader, role, userType) {
        ...
        if (!token || token === 'null' || token === ' ') {
            ...
            return res.status(401).send({ code: 401, message: 'Bearer Token not found' });
        }

        this.verifyJWT(req, res, next, publicKey, permissions, token, role, userType);
    }
    ...
    auth(role, userType) {
        return (req, res, next) => {
            ...
            if (typeof authHeader === 'undefined') {
                ...
                this.extractBearerToken(req, res, next, this.publicKey, null, authHeader, role, userType);
            }
        };
    }
    ...
    authWithPermission(permissions, role) {
        return (req, res, next) => {
            ...
            if (typeof authHeader === 'undefined') {
                ...
                this.extractBearerToken(req, res, next, this.publicKey, permissions, authHeader, role);
            }
        };
    }
}
