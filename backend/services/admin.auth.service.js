...
const { findOneAndDeleteUserRefreshTokenRepo } = require("../data-access/token.repo");
        ...
        switch (userType) {
            ...
            case SETTINGS.USERS.CUSTOMER:
                ...
                if (!user || user.isArchive) {
                    throw { message: "Customer not found!", userCheck: true };
                }
                if (!user.isActive) {
                    throw {
                        message: "Customer has been deactivated. Please contact system admin",
                        activateCheck: true,
                    };
                }
                return await validateUser(password, user, SETTINGS.USERS.CUSTOMER);
        }
    } catch (e) {
...
const validateUser = async (password, user, userType) => {
    try {
        ...
        if (!result) {
            throw { message: "Invalid User name or Password!", credentialCheck: true };
        }
    ...
    try {
        ...
        if (user) {
            ...
            await sendEmailService(
                ...
                {
                    url: `${config.get("frontEndUrl")}/auth/reset-password/${pwResetData.token}`,
                    name: user.firstname + " " + user.lastname,
                },
                email,
                "Password Reset"
    ...
    try {
        ...
        if (user) {
            ...
            await sendEmailService(
                SETTINGS.EMAIL.PASSWORD_RESET,
                {
                    url: `${config.get("webSiteUrl")}/auth/reset-password/${pwResetData.token}`,
                    name: user.firstname + " " + user.lastname,
                },
                email,
                "Password Reset"
